...
app.post('/getcaptions',function(req,res) {
  ...
  console.log("cheak array containr length");
let arraydb = arraycontainer.concat(manullyaddedarray);
let captionstosend  = arraydb.sort(() => Math.random() - Math.random()).slice(0, 25);

if(captionstosend.length >= 0){
  res.json(captionstosend);
}else{
   res.json(["No captions found sorry."]);
}
});

app.get('/loadcaptions',function(req,res){
  console.log("loadcaptions me aaya");
  const options = {
    method: 'GET',
    url: 'https://instagram-captions.p.rapidapi.com/captions',
    headers: {
      'X-RapidAPI-Key': 'a24b4860a7msh57f0a9fe36c434dp1dfcfajsnd2a32f945c83',
  ...
  axios.request(options).then(function (response) {
    ...
    arraycontainer = response.data.captions;
  }).catch(function (error) {
    console.error(error);
  });
});

app.post('/uploadmanullycaption',function(req,res) {
let caption = req.body.caption;
manullyaddedarray.push(caption);
console.log(manullyaddedarray);
res.end("successfull");
});

app.get('/refreshcaptions',function(req,res){
  console.log("loadcaptions me aaya");
  const options = {
    method: 'GET',
    url: 'https://instagram-captions.p.rapidapi.com/captions',
